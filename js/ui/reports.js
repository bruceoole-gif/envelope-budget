...
export function renderReports(root) {
  ...
  const catData = spendingByCategory(from, to);
  ...
  root.innerHTML = `
    ...
    <section class="section">
      <h2>Spending by category <span class="muted">— last 90 days</span></h2>
      ${catData.length === 0 ? emptyState('No expenses yet.', '') : barChart(catData.slice(0, 10))}
    </section>

    <section class="section">
      <h2>Income variability</h2>
      ${incomeData.every((d) => d.value === 0) ? emptyState('No income recorded yet.', '') : barChart(incomeData)}
    </section>
    ...
    <section class="section">
      ...
      ${folders.length === 0 ? emptyState('No folders yet.', '') : `
        <div class="list">
          ${folders.map((f) => `<div class="list-row"><div class="row-main"><span class="row-title">${f.name}</span></div>${sparkline(folderBalanceSeries(f.id), { width: 160, height: 36 })}</div>`).join('')}
        </div>`}
    </section>
